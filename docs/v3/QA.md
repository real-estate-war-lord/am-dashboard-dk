...
| O11 | low | At 390 px the **mini map's legend stack scrolls**: the map is ~175 px tall, the indicator legend alone is ~110 px, and with three feature layers on the last folded card sits below the fold of the stack (it can be scrolled to — the stack takes pointer events — but nothing on screen says so). P10 made this one card worse by adding Services. | The main map folds its whole stack behind a `Legend ▾` pill at that width (§6, P8); the mini map wants the same control, which means a second `legend-toggle` on the page and a decision about which one the ACs mean. More surface than this phase should change. The desktop case is fixed: the stack may take 74 % of the map above 600 px and `lgFitIn()` now folds until the content fits its own box. |
