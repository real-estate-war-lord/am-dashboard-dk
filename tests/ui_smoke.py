...
ROUTES = [
    ...
    # the sidebar footer's Export ▾ is on every route (P7); the Data and test-property headers add a
    # second trigger onto the same one-menu-at-a-time state.
    dict(id="map",            hash="map?ind=growth",                         land=["#lfmap .leaflet-pane", "[data-testid=legend]!", "[data-testid=search]", "[data-testid=layers-btn]!", "[data-testid=ind-picker-btn]!", "[data-testid=ind-chips]!", "[data-testid=period-year]", "[data-testid=map-full]", "#nav", "[data-testid=export-btn]!", "[data-testid=export-menu]!"], state="S.view==='makro' && !!LF.map && window.__maps.length===1 && document.querySelector('[data-testid=map-toolbar] [data-row]').children.length<=6 && document.querySelectorAll('[data-export]').length>=6 && !document.querySelector('[data-xall]')"),
    ...
    dict(id="data_areas",     hash="data/areas/kommune?ind=growth",           land=["[data-testid=data-tabs]", "[data-testid=areas-table] tbody tr", "#tq", "[data-testid=ind-picker-btn]!", "[data-testid=areas-table] th[data-col=growth].on", ".hd-act [data-testid=export-btn]!"], state="S.view==='table' && T.level==='kommune' && document.querySelectorAll('[data-testid=export-btn]').length===2"),
    ...
    dict(id="property",       hash="property?p=55.64250,12.53850:Sydhavn&lay=infra,public,climate&hz=2070", land=["#anmap .leaflet-pane", "#anlegend!", "#anpub!", "#anclim!", "[data-testid=study-row]!", "[data-testid=chart-panel]!", "[data-testid=minimap]!", "[data-testid=minimap-full]", "[data-testid=tiles]!", "[data-testid=prop-input]", "[data-testid=ind-picker-btn]!", "[data-testid=ind-chips]!", "[data-testid=layers-btn]!", "[data-testid=tp-radius]", "[data-testid=tp-sec-infra][open]", "[data-testid=tp-sec-sources]!", ".hd-act [data-export=property]"], wait="#anmap .leaflet-overlay-pane", state="S.view==='analysis' && !!LF.anmap && !!KOM.list && window.__maps.length===1 && AN.show.has('infra') && !document.querySelector('.hd-act [data-export=property][disabled]')", settle=2500, redirect="property?p=55.6425,12.5385:Sydhavn"),
